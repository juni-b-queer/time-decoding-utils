...
describe('extractDateTimeString Test Suite', () => {
    ...
    it('should extract "9/3/24 4PM" from the string with a lot of words', () => {
        const input = "4PM on 9/3/24 a long string of words and stuff that also has the word on or at to throw off and test the functions handling";
...
describe('Testing addLeadingZeros function from time-decoding-utils module', () => {
    ...
    test('should throw an error for invalid date format', () => {
        const input = '7-1';
        expect(() => addLeadingZeros(input)).toThrowError(new Error('Invalid date format'));
    });
});
